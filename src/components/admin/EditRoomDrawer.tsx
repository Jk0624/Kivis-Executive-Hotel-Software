...
const [existingPhotoPublicIds, setExistingPhotoPublicIds] = useState<string[]>([]);
...
const photoPublicIds = [
  ...
  ...uploaded.photoPublicIds,
    ...
    await api.patch(`/admin/rooms/${roomId}`, {
...
const fetchRoom = async () => {
  ...
  try {
    ...
    setExistingPhotoPublicIds(room.photoPublicIds || []);
...
<div>
  ...
  if (index < existingPhotos.length) {
    ...
    setExistingPhotoPublicIds((prev) =>
      prev.filter((_, i) => i !== index)
    );
  } else {
    const newImageIndex = index - existingPhotos.length;
...
<div className="flex justify-end gap-3 border-t p-6">
  ...
  >
    {isUploading ? "Uploading..." : "Update Room"}
  </button>
</div>
      </div>
    </div>
